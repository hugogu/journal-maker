...
5. **Given** 场景分析进行中，**When** 用户刷新页面，**Then** 对话历史自动从浏览器本地存储恢复
6. **Given** 场景已创建，**When** 用户编辑场景描述，**Then** 更新的描述会作为上下文传递给后续AI分析
...
6. **localStorage容量限制**: 当对话历史超过浏览器存储配额时，系统应自动清理旧的对话记录，保留最近20轮对话。
7. **多标签页冲突**: 当用户在不同标签页同时分析同一场景时，localStorage中的对话历史可能产生冲突，以最后一次保存为准。
...
- **FR-016**: 系统必须支持编辑场景描述，并将更新后的描述作为AI分析的上下文
- **FR-017**: AI分析对话历史必须持久化到浏览器本地存储，页面刷新后可恢复
...
- **SC-011**: 场景描述编辑后，AI分析在下次对话中立即使用更新的描述（延迟<1秒）
- **SC-012**: 对话历史从localStorage恢复的延迟不超过500毫秒
