        ...
        >
          <div class="flex items-center mb-2 justify-between">
            <div class="flex items-center">
              ...
              <div class="font-medium text-sm flex items-center gap-2">
                ...
                <!-- Confirmation Badge -->
                <span
                  v-if="message.role === 'assistant' && isConfirmed(message.id)"
                ...
                <span
                  v-if="message.role === 'assistant' && streaming && index === messages.length - 1"
                  class="ml-2 inline-block w-2 h-2 bg-blue-500 rounded-full animate-pulse"
                ></span>
              </div>
            </div>
          ...
          <div class="relative">
            <div
              ...
              :class="{
                'message-collapsed': !isExpanded(index) && shouldShowExpandButton(streamingContent),
              }"
            ...
            <div
              ...
              class="message-content markdown-content"
              :class="{
                'message-collapsed': !isExpanded(index) && shouldShowExpandButton(message.content),
              }"
            ...
            <div
              v-if="
                shouldShowExpandButton(message.content) &&
...
<script setup lang="ts">
  ...
  const scenarioIdNum = computed(() =>
    typeof props.scenarioId === 'string' ? parseInt(props.scenarioId, 10) : props.scenarioId
  )
  ...
  const {
    ...
    loadMessages,
  } = useConversation(scenarioIdNum.value)
  ...
  async function sendMessage() {
    ...
    try {
      ...
      const assistantMessage: { role: 'assistant'; content: string; id?: number } = {
        role: 'assistant',
        content: '',
      }
            ...
            try {
              ...
              if (data.type === 'chunk') {
                ...
                if (
                  ...
                  data.message?.includes('Connect Timeout')
                ) {
                  errorContent += '\n\n**建议:** 检查网络连接或稍后重试'
                }
      ...
      if (
        e instanceof Error &&
        (e.message.includes('fetch failed') || e.message.includes('Connect Timeout Error'))
      ) {
