...
export const analysisDiagramTypeEnum = pgEnum('analysis_diagram_type', [
  'mermaid',
  'chart',
  'table',
])
...
// Journal Rules table
export const journalRules = pgTable(
  'journal_rules',
  {
    id: serial('id').primaryKey(),
  ...
  (table) => [index('idx_journal_rules_scenario_id').on(table.scenarioId)]
)
...
export const sampleTransactions = pgTable(
  'sample_transactions',
  {
    id: serial('id').primaryKey(),
    scenarioId: integer('scenario_id')
  ...
  (table) => [index('idx_sample_transactions_scenario_id').on(table.scenarioId)]
)
...
export const accountMappings = pgTable(
  ...
  (table) => [unique('idx_account_mappings_scenario_account').on(table.scenarioId, table.accountId)]
)
...
export const aiProviders = pgTable(
  ...
  (table) => [index('idx_ai_providers_status').on(table.status)]
)
...
export const aiModels = pgTable(
  ...
  (table) => [unique('idx_ai_models_provider_name').on(table.providerId, table.name)]
)
...
export const analysisEntries = pgTable(
  'analysis_entries',
  {
    id: serial('id').primaryKey(),
    scenarioId: integer('scenario_id')
...
export const analysisDiagrams = pgTable(
  'analysis_diagrams',
  {
    id: serial('id').primaryKey(),
    scenarioId: integer('scenario_id')
...
export const conversationShares = pgTable(
  ...
  (table) => [index('idx_conversation_shares_token').on(table.shareToken)]
)
