  ...
  try {
    ...
    if (method === 'POST') {
      ...
      const existing = await db.query.accounts.findFirst({
        where: and(eq(accounts.companyId, companyId), eq(accounts.code, data.code)),
      })
    ...
    if (method === 'DELETE') {
      ...
      const candidateRules = scenarioIds.length
        ? await db.query.journalRules.findMany({
            where: inArray(
              journalRules.scenarioId,
              scenarioIds.map((s) => s.id)
            ),
          })
        : []
      ...
      const hasAccountReference = (value: any): boolean => {
        ...
        if (typeof value === 'object') {
          if (
            ...
            value.accountCode === account.code ||
      ...
      const structuredRulesUsingAccount = candidateRules.filter(
        (rule) => hasAccountReference(rule.debitSide) || hasAccountReference(rule.creditSide)
      )

      const rulesUsingAccount = [
        ...debitRulesUsingAccount,
        ...creditRulesUsingAccount,
        ...structuredRulesUsingAccount,
      ]

      const usageCount =
        scenariosUsingAccount.length + transactionsUsingAccount.length + rulesUsingAccount.length

      if (usageCount > 0) {
        throw new AppError(
          400,
          `无法删除科目：该科目被 ${usageCount} 个地方使用（场景：${scenariosUsingAccount.length}，交易：${transactionsUsingAccount.length}，规则：${rulesUsingAccount.length}）`
        )
      }
