<template>
  <div class="overflow-x-auto">
        ...
        >
          ...
          <td class="py-2 px-3">
            <span
              ...
              :class="[
                'inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium',
                subject.isExisting ? 'bg-green-100 text-green-800' : 'bg-blue-100 text-blue-800',
              ]"
              :title="subject.isExisting ? '该科目已存在于系统中' : '建议新建的科目'"
            >
              <svg
                v-if="subject.isExisting"
                class="w-3 h-3 mr-1"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
    ...
    <p v-if="subjects.length === 0" class="text-gray-400 text-sm text-center py-4">暂无会计科目</p>
  </div>
</template>
